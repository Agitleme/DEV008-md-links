...
describe('fileToStringArray', () => {
  ...
  it('debe proporcionar un array de objetos (aux3) que incluyan ruta así como contenido', async () => {
    // Función asincrónica que devuelve un array
    const result = await fileToStringArray(aux3);

    //Realizar las comprobaciones de igualdad de manera asíncrona
    expect(result).toStrictEqual(rutaYContenido);
  });
});
